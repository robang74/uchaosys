 ...
 */
#define APPNAME "uchaosd"
#define VERSION "v0.1"
const char USAGE_STRING[] =
    "\nUsage " APPNAME " " VERSION ":\n\n"
    "  /sbin/" APPNAME " [interval_sec] [source_dev] [sink_dev] [watchdog_flag]\n\n"
    "Arguments:\n\n"
 ...
 *
 * Compile:
 *   musl-gcc uchaosd.c -o uchaosd -static -s -Os -Wall
 *
 * Start from /init (example):
 *   /sbin/uchaosd 10 /dev/uchaos /dev/random 1 &
 */
...
int main(int argc, char **argv)
{
    ...
    if (wpet) {
        ...
        if (wdfd < 0) { 
            fprintf(stderr, APPNAME ": cannot open %s: %s\n",
                wdog, strerror(errno));
            wdfd = -1;                        // silently disable if no watchdog
        }
    }
